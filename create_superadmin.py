...
def check_user_exists(username, email):
    ...
    try:
        ...
        with app.app_context():
            ...
            email_user = userModel.query.filter_by(
    ...
    try:
        ...
        with app.app_context():
            ...
            enterprise_plan = PlanModel.query.filter_by(
                plan_type=PlanType.ENTERPRISE, is_active=True
            ).first()
            ...
            pro_plan = PlanModel.query.filter_by(
                plan_type=PlanType.PRO, is_active=True
            ).first()
...
def create_super_admin():
    ...
    if existing_user:
        ...
        if (
            input("\nDo you want to update this user to super admin? (y/N): ").lower()
            == "y"
        ):
            try:
                with app.app_context():
    ...
    try:
        ...
        with app.app_context():
            super_admin = userModel(
                ...
                + timedelta(days=365 * 10),  # 10 years
                is_active=True,
                is_deleted=False,
                meetings=0,
            )
            ...
            print(
                f"   📅 Subscription: Active until {super_admin.subscription_end_date.strftime('%Y-%m-%d')}"
            )
