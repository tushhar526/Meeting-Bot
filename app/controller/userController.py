...
def update_user_profile(user_id, request):
    ...
    if "username" in data:
        if not data["username"] or len(data["username"].strip()) < 3:
            logger.warning(
                f"Profile update failed - invalid username for user {user_id}"
            )
            return (
    ...
    if "email" in data:
        ...
        if existing_user:
            logger.warning(
                f"Profile update failed - email already taken for user {user_id}"
            )
...
def change_password(user_id, request):
    ...
    if len(new_password) < 6:
        logger.warning(
            f"Password change failed - new password too short for user {user_id}"
        )
        return (
...
def get_user_analytics(user_id):
    ...
    try:
        ...
        platform_stats = (
            db.session.query(
                JobModel.platform, func.count(JobModel.job_id).label("count")
            )
            .filter_by(user_id=user_id)
        ...
        for job in recent_meetings:
            ...
            if job.started_at and job.ended_at:
                duration = job.ended_at - job.started_at
                meeting_data["duration_hours"] = round(
...
def get_all_plans(user_id):
    ...
    try:
        ...
        for plan_type in PlanType:
            ...
            # Only include plans that are active in database
            if db_plan and not db_plan.is_active:
                continue

            # Check if plan should be visible to users (handle missing column gracefully)
            if (
                db_plan
                and hasattr(db_plan, "is_visible_to_users")
                and not db_plan.is_visible_to_users
            ):
                continue
            ...
            plans_list.append(
                {
                    ...
                    "features": {
                        feat: PlanConfig.has_feature_access(plan_type, feat)
                        for feat in PlanConfig.FEATURES.keys()
                    },
                    "feature_display_names": PlanConfig.FEATURES,
                }
            )

        current_plan = user.plan.plan_type if user.plan else None
        plan_names = [name["name"] for name in plans_list]
        print(f"WHAT THE HEIL MAN =  {plan_names}")
        ...
        logger.error(
            f"Error getting plans for user {user_id} with problem {str(e)}",
            exception=e,
        )
...
def assign_plan_to_user(user_id, plan_type):
    ...
    try:
        ...
        if (
            user.plan
            and user.plan.plan_type == plan_type_enum.value
            and user.subscription_status == SubscriptionStatus.ACTIVE
        ):
            return (
                jsonify(
                    {"error": f"User is already on the {plan_type_enum.value} plan"}
                ),
                400,
            )
        ...
        if not plan:
            return (
                jsonify(
                    {"error": "Plan not found in database. Please seed the database."}
                ),
                404,
            )
    ...
    try:
        ...
        if user.subscription_end_date:
            now = (
                ...
                if user.subscription_end_date.tzinfo
        ...
        features = [
            {
                "id": k,
                "name": PlanConfig.get_feature_display_name(k),
                "has_access": PlanConfig.has_feature_access(user.plan.plan_type, k),
            }
