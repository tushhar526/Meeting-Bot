    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            try:
                ...
                logger.info(
                    f"Plan access check for user {user_id}, feature: {required_feature}"
                )
                ...
                if not user.has_active_subscription():
                    logger.warning(
                        f"Plan access denied - no active subscription: {user_id}"
                    )
                ...
                #         logger.warning(
                #             f"Plan access denied - meeting limit exceeded: {user_id}"
                #         )
                #         return (
                #             jsonify(
                #                 {
                ...
                if required_feature:
                    ...
                    if required_feature == "recording":
                        if not plan_limits["unlimited_meetings"] and max_meetings is not None:
                            logger.warning(
                                f"Plan access denied - feature '{required_feature}' not available in {user.plan.plan_type} plan for user {user_id}"
                            )
                            return (
                                jsonify(
                                    {
                    ...
                    if not PlanConfig.has_feature_access(
                        user.plan.plan_type, required_feature
                    ):
                        logger.warning(
                            f"Plan access denied - feature '{required_feature}' not available in {user.plan.plan_type} plan for user {user_id}"
                        )
                        return (
                            jsonify(
                                {
                                    "error": f"Feature '{required_feature}' not available in {user.plan.plan_type} plan",
                                    "required_feature": required_feature,
                                    "current_plan": user.plan.plan_type,
                                    "available_features": [
                                        feat
                                        for feat in PlanConfig.get_available_features()
                                        if PlanConfig.has_feature_access(
                                            user.plan.plan_type, feat
                                        )
                                    ],
                                }
                            ),
                ...
                logger.error(
                    f"Plan access check failed for user {user_id} as this happened {e} i did changed the .vlaue stuff man",
                    exception=e,
                )
